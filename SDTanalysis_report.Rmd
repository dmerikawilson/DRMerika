...
```{r oldcode_deletemepleeeeease, eval=FALSE}
# source("WithinSubjectsErrorBars.R")
...
# 
# SEwithinLureDataOld <- summarySEwithin(data= as.data.frame(filter(DRM_subLevel_summary, lure_dist == 'lure')), 
#                                     measurevar = "dprime",
#                                     withinvars= c("list_type", "target_dist", "target_list_length"),
#                                     betweenvars = "group",
#                                     idvar="subject", na.rm=FALSE, conf.interval=.95, .drop=TRUE)
# SEwithinTargetData <- summarySEwithin(data= as.data.frame(filter(DRM_subLevel_summary, target_dist == 'target')), 
#                                     measurevar = "dprime",
#                                     withinvars= c("list_type", "lure_dist", "target_list_length"),
#                                     betweenvars = "group",
#                                     idvar="subject", na.rm=FALSE, conf.interval=.95, .drop=TRUE)
# 
# SEwithinTargetDataSimpleGroup <- summarySEwithin(data = as.data.frame(filter(filter(DRM_subLevel_summary,
#                                                                                            target_dist ==
#                                                                        'target'), lure_dist == 'related' |
#                                                                 lure_dist == 'critical')), measure = "dprime", withinvars = c("lure_dist", "list_type"), betweenvars = "group", idvar = "subject", na.rm = FALSE, conf.interval= .95, .drop=TRUE)
# 
# 
# SEwithinFAData <- summarySEwithin(data= as.data.frame(filter(DRM_subLevel_summary, lure_dist == 'lure'
#                                                              | lure_dist == 'related')), 
#                                     measurevar = "FA", withinvars= c("list_type", "lure_list_length"),
#                                     betweenvars = "group",
#                                     idvar="subject", na.rm=FALSE, conf.interval=.95, .drop=TRUE)
# 
# SEwithinFADataSimpleGroup <- summarySEwithin(data = as.data.frame(filter(DRM_subLevel_summary,
#                                                                                 lure_dist == 'lure' |
#                                                                 lure_dist == 'related')), measure = "FA", withinvars = c("list_type"), betweenvars = "group", idvar = "subject", na.rm = FALSE, conf.interval= .95, .drop=TRUE)

# # New plot with transparencies
# ggplot(data = SEwithinLureData, mapping = aes(x = target_dist, y = dprime)) +
#      geom_bar(aes(alpha = target_list_length, fill = list_type), position = position_dodge(), stat = "identity", color = "black") +
#      geom_errorbar(aes(ymax = dprime +sem, ymin = dprime - sem), position = position_dodge(), width = .2) +
#      scale_alpha_manual(values=(c(0.1,0.5,1))) +
# facet_wrap(~group)
# 
# # Correct color of errors bars, but wrong position
# ggplot(data = SEwithinLureData, mapping = aes(x = target_dist, y = dprime)) +
# geom_bar(aes(alpha = target_list_length, fill = list_type), position = position_dodge(), stat = "identity", color = "black") +
# geom_errorbar(aes(ymax = dprime +sem, ymin = dprime - sem), position = position_dodge(2), width = .2) +
# scale_alpha_manual(values=(c(0.1,0.5,1))) +
# facet_wrap(~group)



# ggplot(data = SEwithinLureData, aes(x = target_dist, y = dprime, 
#                                     fill = interaction(target_list_length, list_type))) + 
#   geom_bar(position = position_dodge(1), stat = "identity", color = "black") + 
#   geom_errorbar(aes(ymax= dprime +sem, ymin = dprime - sem), position = position_dodge(1),
#                 width = .2) + 
#   scale_x_discrete("Item Type", limits = c("related", "critical", "target"),
#                    labels = c("Related Lures", "Critical Lures", "Studied Items")) +
#   scale_fill_manual(values=c("#FF6666", "#FF0000", "#99CC99", "#003300"),
#                              name = "List\nType\nand\nLength") +
#                     # labels = c("")
#   ylab("d'") +
